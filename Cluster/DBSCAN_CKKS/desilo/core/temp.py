...
import numpy as np
# 앞서 변환 완료된 FHE 최적화 모듈들을 임포트한다고 가정합니다.
...
def send_to_server_fhe(
    ...
    encrypted_columns, num_points, eps, min_pts, dimension
):
    ...
    for k in range(1, N):
        ...
        # 🚨 [Probe 1] k=1 일 때 이웃 여부가 0.0과 1.0으로 잘 나오는지 검사
        if k == 1:
            dec_adj = engine.decrypt(adj_k, secret_key)
            print(f"  -> k=1 일 때 이웃 배열 (상위 10개): {np.round(dec_adj[:10], 4)}")

    # 자기 자신 포함
    ones_plaintext = engine.encode([1.0 for _ in range(N)])
    total_neighbors_ct = engine.add(total_neighbors_ct, ones_plaintext)
    
    total_neighbors_ct = engine.bootstrap(total_neighbors_ct, keypack.relinearization_key, keypack.conjugation_key, keypack.bootstrap_key)
