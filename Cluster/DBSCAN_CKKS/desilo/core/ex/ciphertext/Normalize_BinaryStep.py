...
def check_neighbor_closed_interval_heaviside9(
    ...
    lower_abs = abs(-(eps_sq + margin_val))
    upper_abs = abs(max_dist_sq - (eps_sq + margin_val))
    bound = max(lower_abs, upper_abs) * safety_factor

    scale_factor = 1.0 / bound
    scale_pt = engine.encode([scale_factor for _ in range(slot_count)])
    t = engine.multiply(x, scale_pt)

    # 3) 9차 odd polynomial로 sign-like 근사
    # 실험용 baseline:
    # s(t) = a1 t + a3 t^3 + a5 t^5 + a7 t^7 + a9 t^9
    # 이후 step(t) = 0.5 - 0.5*s(t)
    #
    # 주의: 이 계수는 "논문 exact coefficient"가 아니라
    # [-1,1]용 실험 baseline 입니다.
    a1 = 2.4609375
    a3 = -5.7421875
    a5 = 9.84375
    a7 = -7.3828125
    ...
    # 선택적 clipping 대용 affine 안정화는 일단 생략
    # 6) x<=0 -> 1, x>0 -> 0
