...
def main():
    ...
    for x in np_final_labels[:N]:
        r = round(x)
        if r <= 0:
            cluster_labels_np.append(-1)
        elif r > N:
            cluster_labels_np.append(N)
        else:
            cluster_labels_np.append(r)
    ...
    for x in decrypted_labels[:N]:
        r = np.round(x)
        if r <= 0:
            cluster_labels_fhe.append(-1)
        elif r > N:
            cluster_labels_fhe.append(N)
        else:
            cluster_labels_fhe.append(r)
    ...
    if "timings" in debug_fhe:
        ...
        total_stage = (
            debug_fhe['timings'].get('normalize_sec', 0)
            + debug_fhe['timings'].get('core_sec', 0)
            + debug_fhe['timings'].get('label_propagation_sec', 0)
            + debug_fhe['timings'].get('postprocess_sec', 0)
        )
        print(f"합계(서버 주요 단계)      : {total_stage:.2f}초\n")

        timing_filename = f"debug_timings_eps{eps_val}_min{min_pts_val}.txt"
        save_timings_txt(timing_filename, debug_fhe["timings"])

    # ---------------------------------------------------------
    # 6. 중간 단계 값 차이 비교용 디버그 CSV 생성
    # ---------------------------------------------------------
    ...
    try:
        with open(debug_filename, 'w', encoding='utf-8') as f:
            ...
            for i in range(N):
                ...
                f.write(
                    f"{i},"
                    f"{n_np:.4f},{n_fhe:.4f},{d_n:.4f},"
                    f"{c_np:.4f},{c_fhe:.4f},{d_c:.4f},"
                    f"{l_np:.4f},{l_fhe:.4f},{d_l:.4f}\n"
                )
